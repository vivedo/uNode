...
static bool check_uart_no(uint8_t uart_no) {
    if (uart_no >= SOFTUART_MAX_UARTS) {
        ESP_LOGD(TAG, "Invalid uart number %d, %d max", uart_no, SOFTUART_MAX_UARTS);
        return false;
    }
...
static bool check_uart_enabled(uint8_t uart_no) {
    if (!uarts[uart_no].baudrate) {
        ESP_LOGD(TAG, "Uart %d is disabled", uart_no);
        return false;
    }
...
bool softuart_open(uint8_t uart_no, uint32_t baudrate, uint32_t rx_pin, uint32_t tx_pin) {
    ...
    if (baudrate == 0) {
        ESP_LOGD(TAG, "Invalid baudrate");
        return false;
    }
    for (uint8_t i = 0; i < SOFTUART_MAX_UARTS; i++)
        if (uarts[i].baudrate && i != uart_no
            && (uarts[i].rx_pin == rx_pin || uarts[i].tx_pin == tx_pin || uarts[i].rx_pin == tx_pin ||
                uarts[i].tx_pin == rx_pin)) {
            ESP_LOGD(TAG, "Cannot share pins between uarts");
            return false;
        }
...
bool softuart_available(uint8_t uart_no) {
    ...
    return (uart->buffer.receive_buffer_tail + SOFTUART_MAX_RX_BUFF - uart->buffer.receive_buffer_head) %
